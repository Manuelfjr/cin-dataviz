export async function barrasDFI(id) {
    ...
    try {
        ...
        // Definir faixas de DFI
        const bins = [0, 15, 30, 45, 60, 100];
        const labels = ['<15%', '15-30%', '30-45%', '45-60%', '>60%'];

        // Agrupar e processar os dados por DFI
        let groupedData = {};
        data.forEach(d => {
            let dfi = +d['DNA fragmentation index, DFI (%)'];
            let group = labels[bins.findIndex((b, i) => dfi >= b && dfi < bins[i + 1]) - 1];

            if (!groupedData[group]) {
                groupedData[group] = { 'DFI_Group': group, 'Tipo_A': 0, 'Tipo_B': 0, 'Tipo_C': 0, 'Tipo_D': 0, 'Hiperativo': 0, 'Total': 0 };
            }
        ...
        const dadosUsuario = {
            ...
            dfi: idBarras['DNA fragmentation index, DFI (%)']
        };

        console.log(dadosUsuario);
        // Encontrar faixa de DFI do usuário
        const userDFIGroup = labels[bins.findIndex((b, i) => dadosUsuario.dfi >= b && dadosUsuario.dfi < bins[i + 1])];
        ...
        const annotations = ['Tipo_A', 'Tipo_B', 'Tipo_C', 'Tipo_D', 'Hiperativo'].map((tipo, i) => {
            ...
            const valorPadrao = finalData.find(d => d.DFI_Group === userDFIGroup);
